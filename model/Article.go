...
func GetCateArt(id int, pageSize int, pageNum int) ([]Article, int, int) {
	...
	db.Preload("Category").Where(
		"cid = ?", id,
	).Find(&cateArtList).Count(&total).Limit(pageSize).Offset((pageNum - 1) * pageSize)
	//if err != nil {
...
func GetArt(title string, pageSize int, pageNum int) ([]Article, int, int) {
	...
	var total int
	if title == "" {
		db.Preload("Category").Find(&articleList).Count(&total).Limit(pageSize).Offset((pageNum - 1) * pageSize)

	}
	db.Preload("Category").Where(
		"article.title LIKE ?", title+"%",
	).Find(&articleList).Count(&total).Limit(pageSize).Offset((pageNum - 1) * pageSize)
